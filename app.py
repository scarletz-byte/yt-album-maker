...
# FIX 2026: Patch untuk Pillow 10+
import PIL.Image
...
# --- Tampilan UI Glassmorphism ---
st.markdown("""
    <link href="fonts.googleapis.com" rel="stylesheet">
    <style>
    .stApp { background: radial-gradient(circle at top right, #0f172a, #020617); color: #f8fafc; }
    h1 { text-align: center; color: #fbbf24; text-shadow: 0px 0px 15px rgba(251, 191, 36, 0.5); font-family: 'Playfair Display', serif; font-weight: 900 !important; letter-spacing: 2px; }
    div[data-testid="stColumn"] { background: rgba(255, 255, 255, 0.03); border: 1px solid rgba(255, 255, 255, 0.1); padding: 25px; border-radius: 20px; backdrop-filter: blur(10px); }
    .stButton > button { background: linear-gradient(90deg, #fbbf24 0%, #f59e0b 100%); color: #000 !important; border: none; padding: 15px 30px; border-radius: 12px; font-weight: 700; width: 100%; }
    .stButton > button:hover { transform: translateY(-3px); box-shadow: 0px 0px 25px rgba(251, 191, 36, 0.6); }
    [data-testid="stFileUploadDropzone"] { background: rgba(255, 255, 255, 0.05) !important; color: #f8fafc !important; border-color: rgba(255, 255, 255, 0.2) !important; }
    [data-testid="stFileDropzoneInstructions"] > div > span { color: #f8fafc !important; opacity: 0.7; }
    [data-testid="stFileDropzoneInstructions"] > div > small { color: #f8fafc !important; opacity: 0.5; }
    </style>
""", unsafe_allow_html=True)
st.markdown("<h1>🎵 YOUTUBE ALBUM STUDIO 2026</h1>", unsafe_allow_html=True)
st.markdown("<p style='text-align:center; opacity:0.7;'>Automated Video Engine • 720p Optimized • 24 FPS</p>", unsafe_allow_html=True)
st.write("---")
...
if generate_btn:
    if not bg_file or not audio_files:
        ...
        try:
            with st.status("🏗️ Sedang Memproses Album...", expanded=True) as status_box:
                # A. Audio Processing
                ...
                for f in sorted_files:
                    ...
                    track_meta.append({"name": f.name.rsplit('.', 1).replace("_", " ").upper(), "start": curr_ms/1000.0, "end": (curr_ms+len(seg))/1000.0})
                    full_audio += seg
                    curr_ms += len(seg)
                    os.remove(temp_p)
                ...
                with open("bg.png", "wb") as f: f.write(f.getbuffer())
                orig_bg = Image.open("bg.png").convert("RGB").resize((1280, 720))
                dark_bg = (np.array(orig_bg) * 0.3).astype('uint8')
                bg_clip = ImageClip(dark_bg); bg_clip.duration = duration
                # C. Video Compositing
                ...
                for i, tr in enumerate(track_meta):
                    ...
                    t_on = create_text_image(f"{i+1}. {tr['name']}", 20, color=(251, 191, 36), pos=(x_p, y_p), duration=tr['end']-tr['start']).with_start(tr['start'])
                    overlays.extend([t_off, t_on]) 

                # Visualizer Bar (Diperbarui untuk lebar dan transparansi baru)
                def make_spec(t):
                    idx = min(int(t*fps), len(audio_features)-1)
                    frame = np.zeros((150, 1280, 4), dtype=np.uint8)
                    for b in range(50):
                        bh = int(pow(audio_features[idx][b], 0.7) * 130)
                        # Lebar bar: 8 piksel, Jarak: 12 piksel, Transparansi: 200/255
                        frame[150-bh:150, 240+(b*12):240+(b*12)+8] =
                    return frame
                ...
                final.write_videofile("hasil.mp4", fps=fps, codec="libx264", audio_codec="aac", threads=8, preset="medium")

            st.success("✅ Video Berhasil Dibuat!")
            st.markdown("### 📋 Daftar Lagu & Timestamp (Siap Copy)")
            timestamp_text = ""
            for i, tr in enumerate(track_meta):
                start_m = int(tr['start'] // 60)
                start_s = int(tr['start'] % 60)
                start_time_str = f"{start_m:02}:{start_s:02}"
                timestamp_text += f"{i+1}. {start_time_str} - {tr['name']}\n"
            st.code(timestamp_text, language='text')
            with open("hasil.mp4", "rb") as v_file:
                st.download_button("📥 Download MP4 (720p)", v_file, "Album_2026.mp4")
